...
/*
 * sage/gen_exhaustive_groups.sage에 의해 생성된 섹션의 시작이다.
 *
 * 이 섹션은 전수 테스트(exhaustive test)에서 사용하는 작은 위수(order)의 테스트용 
 * 생성점(generator) 상수들을 정의한다.
 *
 * SECP256K1_G_ORDER_7은 위수(order)가 7인 테스트 군(group)에서 사용하는 생성점이며,
 * 이는 Bitcoin mainnet에서 사용하는 일반 secp256k1 생성점 G가 아니라 
 * 가능한 경우를 검사하기 위한 테스트 전용 생성점(generator)이다.
 * 
 * 주어진 위수(order) 7,13,199는 유한체 F_p의 크기가 아니라 
 * 테스트용 생성점 G가 만드는 순환 부분군(cyclic subgroup)의 크기다. 
 *
 * 수식으로 표현하면 아래와 같다.
 *
 * nG = 무한원점(point at infinity) 
 *
 * 즉, 테스트 전용 생성점 G를 n번 더하면 
 * 타원곡선 점군의 항등원인 무한원점에 도달한다.
 * 
 */
...
/* 이 전수 테스트용 군의 위수와 생성점들은 다음 조건을 만족하도록 선택되었다:
 * - 유한체의 크기는 secp256k1과 같으므로 체(field)의 연산 코드는 동일하게 사용된다.
 * - 곡선 방정식은 어떤 작은 상수 B에 대해 y^2 = x^3 + B 형태를 가진다.
 * - 부분군은 생성점 2*P를 가지며, 이때 P.x는 가능한 한 작은 값으로 선택된다.
 * - 부분군의 크기는 전수 테스트를 가능하게 하기 위해 1000보다 작다.
 * - 부분군은 lambda*(x,y) == (beta*x,y) 형태의 자기준동형(endomorphism)을 허용한다.
 *
 * 즉, 쉽게 설명하면 실제 secp256k1과 같은 체 (field) 연산 환경을 유지하되, 
 * 군의 크기는 아주 작게 만들어서 가능한 모든 scalar/point 조합을 전수 테스트
 * 할 수 있게 만든 설정이다.
 */
